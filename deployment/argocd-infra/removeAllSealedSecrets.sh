#!/usr/bin/env bash
# Remove ALL committed *-sealed.yaml under argocd-infra/ — the mirror of
# argocd-infra/sealAllSecrets.sh. Part C: for a clean / vanilla / public git
...
echo "=== Removing argocd-infra sealed secrets (full strip) ==="
find "$SCRIPT_DIR" -name '*-sealed.yaml' | sort | sed 's/^/  - /'
echo ""

if [[ -z "$YES" ]]; then
  read -rp "Permanently remove ALL argocd-infra sealed secrets listed above? [y/N]: " choice
...
if [[ -z "${REMOVE_DEFER_COMMIT:-}" && -z "$SKIP_GIT_COMMIT" ]]; then
  if ! git diff --cached --quiet 2>/dev/null; then
    git commit -m "Remove argocd-infra sealed secrets (full strip)"
    echo "argocd-infra sealed-secret removals committed."
  else
...
echo ""
echo "All argocd-infra sealed secrets removed."
