...
LETS_ENCRYPT_SETTINGS_SCRIPT="$SCRIPT_DIR/letsEncryptSettings.sh"
...
if [ ! -x "$LETS_ENCRYPT_SETTINGS_SCRIPT" ]; then
    echo "Missing or non-executable script: $LETS_ENCRYPT_SETTINGS_SCRIPT"
    exit 1
fi
...
old_lets_encrypt_email="$("$LETS_ENCRYPT_SETTINGS_SCRIPT" get)"
if [ -z "$old_lets_encrypt_email" ]; then
    echo "Could not extract letsEncrypt.email via $LETS_ENCRYPT_SETTINGS_SCRIPT"
    exit 1
fi
...
if [ "$old_lets_encrypt_email" != "$GENERIC_LETS_ENCRYPT_EMAIL" ]; then
    ...
    "$LETS_ENCRYPT_SETTINGS_SCRIPT" set "$GENERIC_LETS_ENCRYPT_EMAIL"
fi

echo "Updating deployment manifests from project_settings.ts with generic release settings..."
"$SCRIPT_DIR/updateArgoCdFromProjectSettings.sh"
...
if [[ "$reply" =~ ^[Yy]$ ]]; then
    ...
    "$LETS_ENCRYPT_SETTINGS_SCRIPT" set "$old_lets_encrypt_email"
