# Updates subdomain/TLD, GitHub repo URL, cert issuer, and openDesk registration template DN from project_settings.ts in ArgoCD, deployment manifests, and scripts/ryax.
...
mapfile -t deployment_files < <(
    find "$REPO_ROOT/deployment" "$REPO_ROOT/ryax_manual" "$REPO_ROOT/scripts/ryax" -type f \
...
issuer_files=(
    ...
    "$REPO_ROOT/scripts/ryax/haproxy-ingress.yaml"
)
...
# Update Ryax TLS environment in prod.yaml.
# letsencrypt-prod -> environment: production (creates letsencrypt-production, uses prod LE ACME server).
# letsencrypt-staging -> environment: stg (creates letsencrypt-stg, uses staging LE ACME server,
#   avoids name collision with the cluster's letsencrypt-staging DNS-01 issuer).
# HAProxy cert is independently issued by the cluster issuer via haproxy-ingress.yaml annotation.
if [ -f "$REPO_ROOT/scripts/ryax/prod.yaml" ]; then
    if [ "$normalized_cert_issuer_type" = "letsencrypt-prod" ]; then
        ryax_env="production"
    else
        ryax_env="stg"
    fi
    sed -i -E "s#^(    environment:)[[:space:]]+\S+(.*)#\1 ${ryax_env}\2#" "$REPO_ROOT/scripts/ryax/prod.yaml"
...
echo "Changed files:"
git --no-pager diff --name-only -- deployment scripts/ryax project_settings.ts | sed '/^$/d' || true
